...
class OWLBP(OWImgWidget):
    settingsList = ['auto_apply', 'block_size']

    def __init__(self, parent=None, signalManager=None):
        super(OWLBP, self).__init__(parent, signalManager)
    ...
    def update_images(self):
        if self.data != None:
            self.images = [OrangeImage(
                ft.local_binary_pattern(
                    color.rgb2gray(image.data), self.points,
                self.radius, self.method)) for image in self.data]
